...
export default observer(function IssueStatusRadar() {
    ...
    function calculateIssuesStatusPercentages(active_or_all_sprints: string) {
        ...
        if (active_or_all_sprints == 'Active') {
            var sprint = issueStore.selectedProject!.sprints.find(
                (sprint) => sprint.is_active === true
            )
            n_issues = n_issues + sprint!.issues.length
            n_todo = sprint!.issues.filter(
                (issue) => issue.status === 'To Do'
            ).length
            n_inprogress = sprint!.issues.filter(
                (issue) => issue.status === 'In Progress'
            ).length
            n_review = sprint!.issues.filter(
                (issue) => issue.status === 'Review'
            ).length
            n_done = sprint!.issues.filter(
            ...
            issueStore.selectedProject!.sprints.map((sprint) => {
                ...
                n_todo =
                    n_todo +
                    sprint.issues.filter((issue) => issue.status === 'To Do')
                        .length
                n_inprogress =
                    n_inprogress +
                ...
                n_review =
                    n_review +
                    sprint.issues.filter((issue) => issue.status === 'Review')
                        .length
                n_done =
                    n_done +
    ...
    return (
        <ResponsiveContainer width="100%" height="100%">
            <RadarChart
                ...
                cy="50%"
                outerRadius="60%"
                data={calculateIssuesStatusPercentages('Active')}
            >
                <PolarGrid
                    textRendering="#FFFFFF"
                    fill="#FFFFFF"
                    stroke="#FFFFFF"
                />
                <PolarAngleAxis
