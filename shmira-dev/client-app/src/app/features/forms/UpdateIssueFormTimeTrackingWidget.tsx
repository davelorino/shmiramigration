...
function convertTimespanToMinutes(timespan: string) {
    ...
    var hours = parseInt(
        timespan.substring(timespan.indexOf('.') + 1, timespan.indexOf(':'))
    )
    ...
    var minutes = parseInt(
        timespan_minus_days_and_hours.substring(
            0,
...
function calculateTrackingBarWidth(
    time_logged: string,
    ...
    if (minutes_remaining !== 0) {
        ...
        console.log(
            Math.round(
                (minutes_logged / (minutes_logged + minutes_remaining)) * 100
            )
        )
        return Math.round(
            (minutes_logged / (minutes_logged + minutes_remaining)) * 100
    ...
    if (minutes_original_estimate !== 0) {
        ...
        console.log(
            Math.min((minutes_logged / minutes_original_estimate) * 100, 100)
        )
        return Math.min(
            (minutes_logged / minutes_original_estimate) * 100,
            100
        ).toString()
    }
}
        ...
        <TrackingWidget>
            ...
            <Right>
                <BarCont>
                    <Bar
                        width={
                            calculateTrackingBarWidth(
                                issueStore.selectedIssue!.time_logged,
                                issueStore.selectedIssue!.time_remaining,
                ...
                <Values>
                    <div>
                        {parseTimeSpan(issueStore.selectedIssue!.time_logged)
                            ? `${parseTimeSpan(
                                  issueStore.selectedIssue!.time_logged
                              )} logged`
                            : 'No time logged'}
                    ...
                    <div>
                        {parseTimeSpan(issueStore.selectedIssue!.time_remaining)
                            ? `${parseTimeSpan(
                                  issueStore.selectedIssue!.time_remaining
                              )} remaining`
                            : 'No time remaining'}
