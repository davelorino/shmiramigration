...
const GanttPage = ({  }) => {
  ...
  const calculatePositionPercentage = (date: string, earliestDate: Date, latestDate: Date): number => {
    ...
    const allStartDates = selectedProject?.sprints.map(sprint => new Date(sprint.date_start).getTime());
    const allEndDates = selectedProject?.sprints.map(sprint => new Date(sprint.date_end).getTime());
    ...
    <GanttContainer>
      ...
      <GanttGrid columns={monthColumns.length}>
        {selectedProject!.sprints.map(sprint => {
          const sprintStartPercentage = calculatePositionPercentage(sprint.date_start, earliestDate, latestDate);
          const sprintEndPercentage = calculatePositionPercentage(sprint.date_end, earliestDate, latestDate);
