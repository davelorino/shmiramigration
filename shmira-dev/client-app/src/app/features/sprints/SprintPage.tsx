...
export default observer(function SprintPage() {
    ...
    var [sprintDetailsExpanded, setSprintDetailsExpanded] = useState<
        Array<boolean>
    >([])
    ...
    const handleIssueDrop = ({
        ...
        var destination_sprint = selectedProject?.sprints.find(
            (sprint) => sprint.id === destination_sprint_id
        )
        ...
        selectedProject!.sprints
            .filter((sprint) => sprint.id === destination_sprint_id)
            .map((sprint) => {
        ...
        selectedProject!.sprints.map((sprint) => {
            ...
            if (sprint.id === destination_sprint_id) {
                sprint.issues
                    ...
                    .map((issue, index) => {
                        console.log(
                            issue.name.concat(' index = ', index.toString())
                        )
                    })
                var splice_index = 0
                if (issueToUpdate!.status == 'To Do') {
        ...
        <>
            <DragDropContext onDragEnd={handleIssueDrop}>
                ...
                {selectedProject!.sprints
                    .filter(
                        (sprint) =>
                            sprint.name !== 'Backlog' && sprint.is_active
                    )
                    .sort(
                        (a: any, b: any) =>
                                ...
                                <div>
                                    {sprint.issues.length === 0 && (
                                        <SprintSectionContainer
                                            sprint={sprint}
                                            index={index}
                                            sprintDetailsExpanded={
                                                sprintDetailsExpanded
                                            }
                                            setSprintDetailsExpanded={
                                                setSprintDetailsExpanded
                                            }
                                        >
                                            <div
