...
const Issues = {
    ...
    updateMultiple: (issues: Issue[], project_id: string) =>
        requests.post(`issues/update_multiple/${project_id}`, issues),
    delete: (id: string) => requests.del(`issues/${id}`),
    addComment: (issue_id: string, comment: Comment) =>
        requests.post(`issues/comment/${issue_id}`, comment),
    addAssigneesToIssue: (issue_assignees: any) =>
        requests.put(`issues/add_assignees_to_issue`, issue_assignees),
    addAssigneeToIssue: (issue_assignee: any) =>
...
const Invites = {
    ...
    decline: (invite_id: string) =>
        requests.get(`invites/decline/${invite_id}`),
}

const Assignees = {
    list: () => requests.get<Assignee[]>(`/assignees`),
    create: (user: Assignee) => requests.post<Assignee>(`/assignees`, user),
    findByAppUserId: (id: string) =>
        requests.get<Assignee>(`/assignees/app_user/${id}`),
}
...
const Accounts = {
    ...
    login: (account: AccountFormValues) =>
        requests.post('/account/login', account),
    update: (account: AccountFormValues) =>
        requests.put('/account/update', account),
    register: (account: AccountFormValues) =>
        requests.post('/account/register', account),
    getactivate: (id: string) => requests.get<Account>(`/account/${id}`),
    postactivate: (account: AccountFormValues) =>
        requests.post<Account>(`/account/activate`, account),
    checkAccountRegistrationStatusByEmail: (account: AccountFormValues) =>
        requests.post(`/account/checkStatus`, account),
...
const Projects = {
    ...
    addSprintToProject: (project_sprint: any) =>
...
const Sprints = {
    ...
    moveIssueToDifferentSprint: (sprint_issue_updated: SprintIssueUpdated) =>
        requests.put(
            `sprints/move_issue_to_different_sprint`,
            sprint_issue_updated
        ),
    closeSprintAndMoveIssuesToBacklog: (
        sprint_to_close: ProjectSprintAndBacklog
