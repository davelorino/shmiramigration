...
const NavBarTop = () => {
    ...
    function renderInviteNotification() {
        if (
            allInvites!.filter(
                (invite) => invite.invitee_account_email === commonStore.email
            ).length > 0
        ) {
            toast(
                <div
                    onClick={() =>
                        modalStore.openModal(<InviteConfirmationForm />)
                    }
                    style={{ display: 'inline' }}
                >
                    {' '}
                    <p>
                        You have an invitation to join a project.
                        <a> Click here </a>to see your invite.
                    </p>
                </div>,
                {
    ...
    function addProjectIdToLegacyTickets() {
        ...
        var issues_to_send: any[] = _.cloneDeep(
            issueStore.selectedProject!.sprints.find(
                (sprint) => sprint.is_active !== true
            )!.issues
        )
        ...
        var createNewProjectOptions = {
            ...
            content: (
                <>
                    <StyledDropdownItem
                        onClick={() =>
                            mediumModalStore.openMediumModal(<ProjectForm />)
                        }
                    >
                        <Icon style={{ marginRight: '0px' }} type="plus" />
                        <ItemText style={{ paddingLeft: '18px' }}>
    ...
    return (
        <Menu fixed="top" primary>
            ...
            >
                <Dropdown.Menu>
                    <Dropdown.Item
                        ...
                        style={{
                            hover: { backdropFilter: 'brightness(120%)' },
            ...
            <Item>
                <Button
                    onClick={() => {
                        issueStore.cancelSelectedIssue()
            ...
            <Menu.Menu position="right">
                ...
                <Avatar
                    onClick={() => {
                        modalStore.openModal(<EditProfileForm />)
                    }}
                    style={{
                        marginTop: '7px',
                        marginRight: '10px',
                    ...
                    round="30px"
                    src={
                        issueStore.selectedProject!.assignees.find(
                            (assignee: Assignee) =>
                    ...
                    name={accountStore.allAccounts
                        .find(
                            (account) => account.id === commonStore.account_id
                        )!
                        .first_name.concat(
                            ' ',
                            accountStore.allAccounts.find(
                                (account) =>
                                    account.id === commonStore.account_id
                            )!.second_name
                        )}
                />
                ...
                {allInvites
                    ...
                    .filter(
                        (invite) =>
                            ...
                            commonStore.email!.toLowerCase()
                    ).length > 0 && renderInviteNotification()}
            </Menu.Menu>
        </Menu>
    )
}
