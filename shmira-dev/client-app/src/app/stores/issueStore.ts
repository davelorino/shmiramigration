...
export default class IssueStore {
    ...
    selectProjectByAssignee = (
        ...
        for (const project of projects) {
            ...
            if (assignee_ids.includes(store.commonStore.assignee_id!)) {
                assignee_name = project.assignees.find(
                    (assignee) => assignee.id == store.commonStore.assignee_id!
                )!.first_name
                    ...
                    current_project!.sprints!.map((sprint) => {
                        ...
                        if (sprint.is_active) {
                            sprint.issues.map((issue) => {
                                var assignees = issue.assignees.filter(
                                    (assignee) =>
                                        this.activeUsers.includes(assignee.id)
                                )
    ...
    createIssue = async (
        ...
        try {
            ...
            await this.loadProjects()
            runInAction(() => {
                var current_project = this.projectRegistry.get(
                    this.selectedProject!.id
                )
                this.tempProject = current_project
                this.selectedProject! = this.tempProject
                ...
                this.selectedProject!.sprints.map((sprint: Sprint) => {
                    if (
                        sprint.id.toLowerCase() ===
                        sprint_issue.sprint_id.toLowerCase()
                    ) {
                        var the_issue = sprint.issues.find(
                            (current_issue: Issue) =>
                                current_issue.id.toLowerCase() ===
    ...
    removeAssigneeFromIssue = async (issue_assignee: IssueAssignee) => {
        ...
        var issue_to_update: any = {
            ...this.issuesByDate.find(
                (issue) => issue.id === issue_assignee.IssueId
            )!,
        }
    ...
    closeSprintAndPushIssuesToBacklog = async (
        project_sprint: ProjectSprintAndBacklog
    ) => {
        this.loading = true
        try {
            await agent.Sprints.closeSprintAndMoveIssuesToBacklog(
                project_sprint
            )
            await this.loadProject(this.selectedProject!.id)
            runInAction(() => {
    ...
    updateIssues = async (issuesToSend: any[]) => {
        this.loading = true
        try {
            var project_returned = await agent.Issues.updateMultiple(
                issuesToSend,
                this.selectedProject!.id
            )
