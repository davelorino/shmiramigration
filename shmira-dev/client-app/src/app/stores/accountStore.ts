...
export default class AccountStore {
    ...
    loadAccounts = async () => {
        try {
            ...
            if (store.commonStore.account_id !== null) {
                this.account = this.accountRegistry.get(
                    store.commonStore.account_id
                )!
            }
    ...
    login = async (creds: AccountFormValues) => {
        try {
            ...
            runInAction(() => {
                store.issueStore.selectProjectByAssignee(
                    user_assignee.id,
                    projects,
                    user.token
                )
    ...
    sendCollaborationEmail = (
        ...
        user_sending_invite: Account
    ) => {
        form.current.from_name.value = user_sending_invite.first_name.concat(
            ' ',
            user_sending_invite.second_name
        )
    ...
    sendRegisterAndCollaborateEmail = (
        e: any,
        form: any,
        generated_link: string
    ) => {
    ...
    invite = async (
        e: any,
        form: any,
        invitee_email: AccountFormValues,
        invitation: Invitation
    ) => {
        try {
            var isUserRegistered =
                await agent.Accounts.checkAccountRegistrationStatusByEmail(
                    invitee_email
                )
            var user_sending_invite = await agent.Accounts.details(
            ...
            runInAction(() => {
                if (isUserRegistered === 'User registered') {
                    ...
                    var generated_link = base_link.concat(
                        '/invites/accept/',
                        invitation.id
                    )
                    this.sendCollaborationEmail(
                        e,
                        form,
                        generated_link,
                        user_sending_invite
    ...
    updateAccountInformation = async (account: Account) => {
        try {
            ...
            await store.issueStore.loadProject(
                store.issueStore.selectedProject!.id
            )
            runInAction(() => {
                //
            })
