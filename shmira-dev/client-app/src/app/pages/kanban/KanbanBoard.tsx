...
import IssueLists from '../../features/lists/IssueLists'
import IssueListsNoActiveSprint from '../../features/lists/IssueListsNoActiveSprint'
...
export default observer(function IssuesDashboard() {
    ...
    const { selectedProject, issuesByDate, updateIssueAndSprint } = issueStore
    ...
    const getDaysRemaining = () => {
        var date_end = moment(
            selectedProject!.sprints.find((sprint) => sprint.is_active)!
                .date_end
        )
        return concat(
    ...
    const handleIssueDrop = ({
        ...
        var destination_sprint_name = destination!.droppableId.substring(
        ...
        selectedProject!.sprints
            ...
            .map((sprint) => {
                number_of_todo = sprint.issues.filter(
                    (issue) => issue.status == 'To Do'
                ).length

                number_of_inprogress = sprint.issues.filter(
                    (issue) => issue.status == 'In Progress'
                ).length

                number_of_review = sprint.issues.filter(
                ...
                if (
                    number_of_inprogress > 0 &&
                    issueToUpdate!.status == 'In Progress'
                ) {
        ...
        if (issueToUpdate!.status == 'In Progress') {
            issueToUpdate!.sort_order = parseInt(
                '20'.concat((destination!.index + number_of_todo).toString())
            )
        }
                ...
                sprint.issues
                    ...
                    .map((issue, index) => {
                        ...
                        if (issue.status == 'In Progress') {
                            issue.sort_order = parseInt(
                                '20'.concat(index.toString())
                            )
                        }
                        if (issue.status == 'Review') {
                            issue.sort_order = parseInt(
                                '300'.concat(index.toString())
                            )
                        }
                        if (issue.status == 'Done') {
                            issue.sort_order = parseInt(
                                '4000'.concat(index.toString())
                            )
                        }
                    })
    ...
    return (
        <div>
            <DragDropContext onDragEnd={handleIssueDrop}>
                    ...
                    .map((sprint) => (
                        <div key={sprint.id} style={{ marginLeft: 40 }}>
                            <div key={sprint.id}>
                                ...
                                >
                                    {moment(sprint.date_start.substr(0, 10))
                                        .format('MMM Do')
                                        .concat(' - ')}
                                    {moment(
                ...
                {selectedProject?.sprints.filter(
                    (sprint) => sprint.is_active === true
                ).length === 0 && <IssueListsNoActiveSprint />}
            </DragDropContext>
        </div>
    )
})
